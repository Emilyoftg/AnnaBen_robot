...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer(f"⚠️ Hey {query.from_user.first_name} You are using this for one of my old messages, send the request again ⚠️",show_alert=True)
        return
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            fek = await message.reply_photo(photo="https://telegra.ph/file/a676fd59d58edb2aab7fe.jpg", caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(300)
            await fek.delete()
            await msg.delete()
    else:
        fuk = await message.reply_photo(photo="https://telegra.ph/file/a676fd59d58edb2aab7fe.jpg", caption=cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(300)
        await fuk.delete()
        await msg.delete()
    if spoll:
